...
def parse_args():
    parser = argparse.ArgumentParser(description='Blockchain Flask App with Command-Line Interface')

    parser.add_argument('--create-blockchain', action='store_true', help='Create a new blockchain')
    parser.add_argument('--delete-blockchain', action='store_true', help='Delete a blockchain')
    parser.add_argument('--store-hashed', action='store_true', help='Store hashed data in blockchain')
    parser.add_argument('--store', action='store_true', help='Store data in blockchain')
    parser.add_argument('--delete-reference', action='store_true', help='Delete reference by ID')
    parser.add_argument('--update-block', action='store_true', help='Update block by ID')
    parser.add_argument('--update-criteria', action='store_true', help='Update block by criteria')
    parser.add_argument('--search', action='store_true', help='Search in blockchain')
    parser.add_argument('--verify', action='store_true', help='Verify blockchain integrity')

    parser.add_argument('--blockchain-name', help='Name of the blockchain')
    parser.add_argument('--block-id', type=int, help='ID of the block')
    parser.add_argument('--criteria', help='Search criteria')
    parser.add_argument('--value', help='Search value')
    parser.add_argument('--new-data', help='New data for updating block')
    parser.add_argument('--reference', help='Reference field for storing data')
...
def delete_blockchain(args):
    blockchain_name = args.blockchain_name

    return delete_blockchain(blockchain_name)


def store_in_blockchain_hashed(args):
    blockchain_name = args.blockchain_name
    data_to_store = args.data
    reference = args.reference

    return store_in_blockchain_hashed(blockchain_name, data_to_store, reference)


def store_in_blockchain(args):
    blockchain_name = args.blockchain_name
    data_to_store = args.data
    reference = args.reference

    return store_in_blockchain(blockchain_name, data_to_store, reference)


def delete_reference_by_id(args):
    blockchain_name = args.blockchain_name
    block_id = args.block_id

    return delete_reference_by_id(blockchain_name, block_id)


def update_block_by_id(args):
    blockchain_name = args.blockchain_name
    block_id = args.block_id
    new_data = args.new_data

    return update_block_by_id(blockchain_name, block_id, new_data)


def update_block_by_criteria(args):
    blockchain_name = args.blockchain_name
    criteria = args.criteria
    value = args.value
    new_data = args.new_data

    return update_block_by_criteria(blockchain_name, criteria, value, new_data)


def search_blockchain(args):
    blockchain_name = args.blockchain_name
    criteria = args.criteria
    value = args.value
...
def main():
    ...
    if args.create_blockchain:
        result = create_blockchain(args)
    elif args.delete_blockchain:
        result = delete_blockchain(args)
    elif args.store_hashed:
        result = store_in_blockchain_hashed(args)
    elif args.store:
        result = store_in_blockchain(args)
    elif args.delete_reference:
        result = delete_reference_by_id(args)
    elif args.update_block:
        result = update_block_by_id(args)
    elif args.update_criteria:
        result = update_block_by_criteria(args)
    elif args.search:
        result = search_blockchain(args)
    elif args.verify:
        result = verify_blockchain(args)
    else:
        result = {'error': 'No valid command specified. Use --help for usage information.'}

    # Wait for Flask thread to finish before exiting
    flask_thread.join()
